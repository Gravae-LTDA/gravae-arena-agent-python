...
class PilotApiUpgrade(unittest.TestCase):
    ...
    def test_success_only_restarts_agent_and_preserves_baseline(self):
        ...
        with patch.object(upgrade.os, 'geteuid', return_value=0), patch.object(upgrade, 'snapshot', return_value=baseline), patch.object(upgrade, 'run') as run, patch.object(upgrade.urllib.request, 'urlopen', return_value=io.BytesIO(json.dumps({'version': '4.0.4'}).encode())) as request, patch('builtins.print'):
            upgrade.main()
